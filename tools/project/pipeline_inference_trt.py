...
def rfdetr_preprocess(img, target_height, target_width):
    """
    Preprocessing for RF-DETR model.
    Direct bilinear resize to target size (no letterbox/padding) + ImageNet normalization.
...
def det_postprocess(output, scale, pad_w, pad_h, orig_shape, target_w=0, target_h=0, conf_thres=0.25, force_resize=False, num_select=300):
    ...
    1. Old format (letterbox): output is [1, N, 6] or [N, 6] where columns are [x1, y1, x2, y2, conf, cls_id]
       - Uses padding compensation: orig_coord = (coord - pad) / scale
    2. RF-DETR format (force_resize): output is tuple (dets, labels) where:
       - dets: [1, 300, 4] in normalized [cx, cy, w, h] format
       - labels: [1, 300, num_classes] raw logits
       - Boxes scaled directly to original image size: xyxy * [orig_w, orig_h, orig_w, orig_h]
...
def main():
    ...
    parser.add_argument('--force-resize', action='store_true',
                        help='Use direct resize preprocessing (no letterbox padding), matching RF-DETR export')
    ...
    for img_path in img_paths:
        ...
        process_img(img_orig, img_path, det_engine, det_input_w, det_input_h,
                    kpt_engine, kpt_input_size, args, output_dir, False, args.force_resize)
        if args.flip:
            img_flip = cv2.flip(img_orig, 1)
            process_img(img_flip, img_path, det_engine, det_input_w, det_input_h,
                        kpt_engine, kpt_input_size, args, output_dir, True, args.force_resize)
...
def process_img(img, img_path, det_engine, det_input_w, det_input_h,
                kpt_engine, kpt_input_size, args, output_dir, is_flip, force_resize=False):
    img_h, img_w = img.shape[:2]
    suffix = '_flip' if is_flip else ''

    if force_resize:
        det_input = rfdetr_preprocess(img, det_input_h, det_input_w)
