...
def run_compute(p_label, query_label, do_output=True, k=5):
    ...
    tp5_num = np.unique(np.where(p_label[:, :k] == query_label[:, np.newaxis])[0]).shape[0]
    if do_output:
...
def compute_acc_by_cat(p_label, query_label, class_list, label_map=None):
    ...
    for cat, data_num in val_dict.items():
        ...
        cat_res = {"top1_num": top1_num, "top1_acc": top1_num/data_num, "top5_num": top5_num, "top5_acc": top5_num/data_num, "data_num": data_num}
...
def choose_with_static(matrix, labels, choose_ratio, use_gpu=False, update_times=0):
    ...
    for cat in cats:
        ...
        for i, (search_cat, count) in enumerate(first_count[:5]):
            cat_static.update({f"top{i+1}_name": search_cat, f"top{i+1}_num": count, f"top{i+1}_ratio": count/search_num})
...
def run_choose(matrix, labels, args):
    if args.remove_mode == "noise":
        ...
        return choose_with_static(matrix, labels, args.threshold, use_gpu=args.use_gpu, update_times=args.update_times)
    else:
