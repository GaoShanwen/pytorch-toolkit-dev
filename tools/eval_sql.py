...
if __name__ == '__main__':
    ...
    sql_server = MySQLHelper(
        host="balance-open13.mysql.rds.aliyuncs.com",
        port=3350,
        user="rueread",
        password="read1107!@#$",
        database="balance1345",
    ...
    for idx, instance in enumerate(read_res):
        ...
        if not len(pred_res):
            continue
        predicts[idx, :len(pred_res)] = np.array([pred["label"] for pred in pred_res])
        gts[idx] = instance[gt_column]
    masks = np.isin(gts, [0])
