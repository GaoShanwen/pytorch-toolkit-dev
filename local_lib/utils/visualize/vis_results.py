...
class VisualizeResults:
    def __init__(
        ...
        self.vis_function = eval(f"vis_one_{task}")
    ...
    def vis_one_search(self, q_file, q_name, pred_files, pred_names, scores=None):
        ...
        for i, (pred_file, p_name) in enumerate(zip(pred_files, pred_names)):
            ...
            if scores is not None:
                img = vis_text(img, f"{scores[i]:.5f}", [2, H - self.text_size - 2], color, self.text_size)
    ...
    def vis_one_classify(self, q_file, q_name, pred_files, pred_names, scores=None):
        if not os.path.exists(q_file):
            return None
        set_height, set_width = self.size
        img = cv2.imread(q_file)
    ...
    def do_visualize(self, gt_labels, gt_files, p_labels, p_files=None, scores=None) -> np.ndarray:
        ...
        for gt_label, gt_file, p_label, p_file, score in zip(gt_labels, gt_files, p_labels, p_files, scores):
            if self.class_map is not None:
                ...
                p_labels = [self.class_map[p] for p in p_labels]
